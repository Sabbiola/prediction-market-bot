...
from .models import LinkageCheckpoint

RAW_FILES = ("linkage_candidates",)
NORMALIZED_FILES = ("linkage_results",)
...
class LinkageLayout:
    ...
    def from_base(
        ...
        *,
        ...
        root_dir = base_dir / linkage_id
        checkpoints_dir = root_dir / "checkpoints"
        resolved_checkpoint = checkpoint_path or checkpoints_dir / "linkage_checkpoint.json"
        return cls(
...
class LinkageStorage:
    def __init__(self, layout: LinkageLayout) -> None:
        self.layout = layout
    ...
    def load_checkpoint(self, *, linkage_id: str, dataset_id: str) -> LinkageCheckpoint:
        if not self.layout.checkpoint_path.exists():
            return LinkageCheckpoint(linkage_id=linkage_id, dataset_id=dataset_id)
        with self.layout.checkpoint_path.open("r", encoding="utf-8") as handle:
            raw = json.load(handle)
        if not isinstance(raw, Mapping):
            return LinkageCheckpoint(linkage_id=linkage_id, dataset_id=dataset_id)
        return LinkageCheckpoint.from_dict(raw, default_linkage_id=linkage_id, default_dataset_id=dataset_id)

    def save_checkpoint(self, checkpoint: LinkageCheckpoint) -> None:
    ...
    def write_manifest(self, payload: Mapping[str, Any]) -> Path:
        path = self.layout.root_dir / "linkage_manifest.json"
