...
from .models import HistoricalIngestCheckpoint
...
class DatasetLayout:
    ...
    @classmethod
    def from_base(cls, *, base_dir: Path, dataset_id: str, checkpoint_path: Path | None = None) -> "DatasetLayout":
        root = base_dir / dataset_id
        checkpoints_dir = root / "checkpoints"
        resolved_checkpoint = checkpoint_path if checkpoint_path is not None else checkpoints_dir / "ingest_checkpoint.json"
        return cls(
...
class DatasetStorage:
    ...
    def load_checkpoint(self, *, dataset_id: str) -> HistoricalIngestCheckpoint:
        path = self.layout.checkpoint_path
        if not path.exists():
            return HistoricalIngestCheckpoint(dataset_id=dataset_id)
        with path.open("r", encoding="utf-8") as handle:
            raw = json.load(handle)
        if not isinstance(raw, dict):
            return HistoricalIngestCheckpoint(dataset_id=dataset_id)
        return HistoricalIngestCheckpoint.from_dict(raw, default_dataset_id=dataset_id)

    def save_checkpoint(self, checkpoint: HistoricalIngestCheckpoint) -> None:
    ...
    def write_manifest(self, payload: Mapping[str, Any]) -> Path:
        manifest = self.layout.root_dir / "dataset_manifest.json"
        manifest.parent.mkdir(parents=True, exist_ok=True)
        manifest.write_text(json.dumps(dict(payload), indent=2), encoding="utf-8")
        return manifest
