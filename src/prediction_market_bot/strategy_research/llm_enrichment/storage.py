...
from .models import LlmEnrichmentCheckpoint

RAW_FILES = ("prompt_requests", "provider_outputs")
NORMALIZED_FILES = ("enrichment_records",)


@dataclass(slots=True, frozen=True)
class LlmEnrichmentLayout:
    enrichment_id: str
    root_dir: Path
    raw_dir: Path
    normalized_dir: Path
    ...
    def from_base(
        ...
        *,
        ...
        root_dir = base_dir / enrichment_id
        checkpoints_dir = root_dir / "checkpoints"
        resolved_checkpoint = checkpoint_path or checkpoints_dir / "llm_enrichment_checkpoint.json"
        return cls(
...
class LlmEnrichmentStorage:
    def __init__(self, layout: LlmEnrichmentLayout) -> None:
        self.layout = layout
    ...
    def load_checkpoint(self, *, enrichment_id: str, linkage_id: str) -> LlmEnrichmentCheckpoint:
        if not self.layout.checkpoint_path.exists():
            return LlmEnrichmentCheckpoint(enrichment_id=enrichment_id, linkage_id=linkage_id)
        with self.layout.checkpoint_path.open("r", encoding="utf-8") as handle:
            raw = json.load(handle)
        if not isinstance(raw, Mapping):
            return LlmEnrichmentCheckpoint(enrichment_id=enrichment_id, linkage_id=linkage_id)
        return LlmEnrichmentCheckpoint.from_dict(
            raw,
            default_enrichment_id=enrichment_id,
            default_linkage_id=linkage_id,
        )

    def save_checkpoint(self, checkpoint: LlmEnrichmentCheckpoint) -> None:
    ...
    def write_manifest(self, payload: Mapping[str, Any]) -> Path:
        path = self.layout.root_dir / "llm_enrichment_manifest.json"
