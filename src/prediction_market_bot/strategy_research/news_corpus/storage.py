...
from .models import NewsCorpusCheckpoint

RAW_FILES = ("news_payloads",)
NORMALIZED_FILES = ("news_articles",)


@dataclass(slots=True, frozen=True)
class NewsCorpusLayout:
    corpus_id: str
    root_dir: Path
    raw_dir: Path
    normalized_dir: Path
    ...
    def from_base(
        ...
        *,
        ...
        resolved_checkpoint = checkpoint_path or checkpoints_dir / "news_checkpoint.json"
        return cls(
            corpus_id=corpus_id,
            root_dir=root_dir,
...
class NewsCorpusStorage:
    def __init__(self, layout: NewsCorpusLayout) -> None:
        self.layout = layout
    ...
    def load_checkpoint(self, *, corpus_id: str, source_id: str) -> NewsCorpusCheckpoint:
        if not self.layout.checkpoint_path.exists():
            return NewsCorpusCheckpoint(corpus_id=corpus_id, source_id=source_id)
        with self.layout.checkpoint_path.open("r", encoding="utf-8") as handle:
            raw = json.load(handle)
        if not isinstance(raw, Mapping):
            return NewsCorpusCheckpoint(corpus_id=corpus_id, source_id=source_id)
        return NewsCorpusCheckpoint.from_dict(
            raw,
            default_corpus_id=corpus_id,
            default_source_id=source_id,
        )

    def save_checkpoint(self, checkpoint: NewsCorpusCheckpoint) -> None:
    ...
    def write_manifest(self, payload: Mapping[str, Any]) -> Path:
        path = self.layout.root_dir / "news_manifest.json"
