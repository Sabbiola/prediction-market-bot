...
def bootstrap_operational_postgres_schema(dsn: str, *, dialect: MigrationDialect = "postgres") -> None:
    upgrade_operational_schema(dsn, dialect=dialect)
...
class PostgresRunsRepository(_PostgresBaseRepository, RunsRepositoryPort):
...
class PostgresReviewQueueRepository(_PostgresBaseRepository, ReviewQueueRepositoryPort):
    ...
    def get_candidate(self, queue_id: str) -> TradeReviewCandidate | None:
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT payload_json FROM review_queue WHERE queue_id = %s",
                    (target,),
                )
                row = cur.fetchone()
        if row is None:
            return None
        try:
            return TradeReviewCandidate.model_validate(self._loads(str(row[0])), strict=False)
        except Exception:
            return None
    ...
    def list_candidates(self, *, run_id: str | None = None, limit: int = 0) -> list[TradeReviewCandidate]:
        ...
        for row in rows:
            try:
                parsed = TradeReviewCandidate.model_validate(self._loads(str(row[0])), strict=False)
            except Exception:
                continue
...
class PostgresReviewDecisionRepository(_PostgresBaseRepository, ReviewDecisionRepositoryPort):
    ...
    def list_decisions(self, *, queue_id: str | None = None, run_id: str | None = None) -> list[TradeReviewDecision]:
        ...
        for row in rows:
            try:
                parsed = TradeReviewDecision.model_validate(self._loads(str(row[0])), strict=False)
            except Exception:
                continue
            records.append(parsed)
        return records
...
class PostgresOpenPositionsRepository(_PostgresBaseRepository, OpenPositionsRepositoryPort):
    ...
    def load_snapshot(self) -> Mapping[str, Any] | None:
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT payload_json FROM open_positions_state WHERE id = 1")
                row = cur.fetchone()
        if row is None:
            return None
...
class PostgresPendingSettlementsRepository(_PostgresBaseRepository, PendingSettlementsRepositoryPort):
    ...
    def get_request(self, request_id: str) -> PendingSettlementRequest | None:
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT payload_json FROM pending_settlements WHERE request_id = %s",
                    (target,),
                )
                row = cur.fetchone()
        if row is None:
            return None
        try:
            return PendingSettlementRequest.model_validate(self._loads(str(row[0])), strict=False)
        except Exception:
            return None
    ...
    def list_requests(
        ...
        *,
        ...
        for row in rows:
            try:
                parsed = PendingSettlementRequest.model_validate(self._loads(str(row[0])), strict=False)
            except Exception:
                continue
            records.append(parsed)
        return records
...
class PostgresTransactionIntentsRepository(_PostgresBaseRepository, TransactionIntentsRepositoryPort):
    ...
    def get_intent(self, intent_id: str) -> TxIntent | None:
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT payload_json FROM transaction_intents WHERE intent_id = %s",
                    (target,),
                )
                row = cur.fetchone()
        if row is None:
            return None
        try:
            return TxIntent.model_validate(self._loads(str(row[0])), strict=False)
        except Exception:
            return None
    ...
    def list_intents(self, *, run_id: str | None = None, limit: int = 0) -> list[TxIntent]:
        ...
        for row in rows:
            try:
                parsed = TxIntent.model_validate(self._loads(str(row[0])), strict=False)
            except Exception:
                continue
...
class PostgresTransactionAttemptsRepository(_PostgresBaseRepository, TransactionAttemptsRepositoryPort):
    ...
    def list_attempts(
        ...
        *,
        ...
        for row in rows:
            try:
                parsed = TxAttempt.model_validate(self._loads(str(row[0])), strict=False)
            except Exception:
                continue
...
class PostgresTransactionReceiptsRepository(_PostgresBaseRepository, TransactionReceiptsRepositoryPort):
    ...
    def list_receipts(
        ...
        *,
        ...
        for row in rows:
            try:
                parsed = TxReceipt.model_validate(self._loads(str(row[0])), strict=False)
            except Exception:
                continue
...
class PostgresOperatorControlStateRepository(_PostgresBaseRepository, OperatorControlStateRepositoryPort):
    def load_state(self) -> Mapping[str, Any] | None:
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT payload_json FROM operator_control_state WHERE id = 1")
                row = cur.fetchone()
        if row is None:
            return None
        return self._loads(str(row[0]))
...
class PostgresOperationalRepositories(OperationalRepositories):
    @classmethod
    def bootstrap(
        cls,
        dsn: str,
        *,
        ...
        dialect: MigrationDialect = "postgres",
    ) -> "PostgresOperationalRepositories":
        if apply_migrations:
            bootstrap_operational_postgres_schema(dsn, dialect=dialect)
        return cls(
            runs=PostgresRunsRepository(dsn),
            review_queue=PostgresReviewQueueRepository(dsn),
            review_decisions=PostgresReviewDecisionRepository(dsn),
            open_positions=PostgresOpenPositionsRepository(dsn),
            pending_settlements=PostgresPendingSettlementsRepository(dsn),
            transaction_intents=PostgresTransactionIntentsRepository(dsn),
            transaction_attempts=PostgresTransactionAttemptsRepository(dsn),
            transaction_receipts=PostgresTransactionReceiptsRepository(dsn),
            operator_control_state=PostgresOperatorControlStateRepository(dsn),
        )
