...
10. training offline baseline:
   ...
   - per run deterministici dove non vuoi dipendere da extras opzionali: `train-baseline-models --dataset-id <dataset_id> --no-xgboost --json`
...
Note ambiente training lab:

- per abilitare candidate XGBoost serve installare extras research: `pip install \".[research]\"`
- se XGBoost non e installato o fallisce a runtime, il run resta esplicito:
  - modello `xgboost_candidate` con `status=skipped`
  - warning testuale nel `summary.json` e output CLI
- la pipeline non deve degradare in modo silenzioso: verificare sempre `models.<name>.status` e `warnings` nel training summary
