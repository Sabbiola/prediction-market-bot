...
Comportamento capability-gated XGBoost:

- se `xgboost` non e installato, il training continua sui baseline e marca `xgboost_candidate` come `status=skipped` con reason esplicita
- se il training XGBoost fallisce a runtime (fit/compatibilita ambiente), il run non degrada in modo silenzioso: `xgboost_candidate` resta `status=skipped` con warning esplicito in summary
- per run deterministici cross-environment (es. CI locale senza extras), usare `train-baseline-models --no-xgboost`
