...
Per la prova operativa staging end-to-end, usare anche il comando orchestrato:

```bash
python -m prediction_market_bot.main beta-dress-rehearsal --config config/app.staging.yaml --agents-config config/agents.yaml --ui-username <user> --ui-password <password> --json
```

Il payload restituisce sempre:

- `go_no_go=GO|NO_GO`
- `steps[]` (13 step del rehearsal con `ok/detail/recovery_hint`)
- `failed_steps[]`
- `evidence_path` (artifact JSON riusabile nel change record)

## Checklist di Rilascio (PASS/FAIL)

1. Startup validation passa con profilo `config/app.staging.yaml` (`validate-startup`) senza errori bloccanti.
   - include check `staging_profile_*` tutti `ok=true`.
2. Dress rehearsal orchestrato (`beta-dress-rehearsal`) restituisce `go_no_go=GO` e `failed_steps=[]`.
   - in caso `NO_GO`, usare `recovery_hint` per fix e ripetere il rehearsal completo.
3. Suite acceptance beta-live verde (`make ci-beta-acceptance`).
4. I test confermano che in `PAPER_LIVE`/`SANDBOX_CHAIN` l’esecuzione non parte senza `APPROVED`.
5. I test confermano persistenza di `model_rationale` e `operator_rationale`.
6. I test confermano che le open positions sopravvivono ai restart.
7. I test confermano che la settlement lane chiude posizioni in passaggio separato.
8. I test confermano tx sandbox tracciabile (`run_id` + `review_queue_id`) e riconciliabile.
9. Replay e report generation da artefatti persistiti risultano operativi.
10. I test UI confermano operativita end-to-end su `Review Queue` e `Sandbox TX`, inclusi path bloccati e audit trail azioni.
11. I test UI confermano monitoraggio operativo (widget live + stale-data counters), banner incident e feed eventi audit.
12. Il tab `Reports` espone shortcut replay/evaluation consistenti con i comandi CLI documentati.
13. Il control plane UI e deployabile separatamente in staging (`prediction-market-ui`) con health/readiness verdi.
14. La acceptance suite include il path UI: login -> overview read -> review approve -> sandbox tx reconcile view.
15. Promotion gate v2 verificato:
...
18. Alt-data + LLM promoted path (sandbox-only) verificato:
...
19. Review Queue UI workflow verificato:
...
20. Incident & historical-debug workflow verificato:
...
Comando consigliato (single-shot):

```bash
python -m prediction_market_bot.main beta-dress-rehearsal --config config/app.staging.yaml --agents-config config/agents.yaml --run-id beta-dress-<date> --operator-id <operator> --approval-rationale "<rationale>" --ui-username <user> --ui-password <password> --json
```

Regola di decisione:

- `go_no_go=GO`: candidato al go-live beta (fermi restando checklist e change review)
- `go_no_go=NO_GO`: release bloccata, usare `failed_steps` + `recovery_hint` per triage
