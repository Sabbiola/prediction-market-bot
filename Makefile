...
.PHONY: install lint typecheck test test-all beta-acceptance beta-dress-rehearsal run smoke-dry-run ci-quality ci-smoke ci-beta-acceptance ci format
...
beta-dress-rehearsal:
	python -m prediction_market_bot.main beta-dress-rehearsal --config config/app.staging.yaml --agents-config config/agents.yaml --json
