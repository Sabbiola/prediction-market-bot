...
def test_beta_acceptance_beta_dress_rehearsal_command_emits_go_no_go_payload(
    temp_config_paths: tuple[Path, Path],
    deterministic_run_id: str,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
    ...
    _configure_ui_auth(app_cfg)
    _patch_live_providers(monkeypatch)
    monkeypatch.setenv("PM_BOT_UI_SESSION_SECRET", "test-session-secret-for-acceptance")

    with _local_sandbox_rpc_server() as rpc_url:
        _configure_app_for_sandbox_chain(app_cfg, rpc_url=rpc_url, provider_selection="AUTO")
        run_id = f"{deterministic_run_id}-dress-gate"

        dress_exit, dress_out = _run_cli(
            [
                ...
                "beta-dress-operator",
                "--approval-rationale",
                "Approved in full sandbox-live dress rehearsal.",
                "--ui-username",
                "operator",
                "--ui-password",
                "operator-pass",
        ...
        _must_ok(dress_exit, dress_out, "beta-dress-rehearsal command failed.")
        payload = json.loads(dress_out)
        _must(payload.get("ok") is True, "beta-dress-rehearsal returned ok=false.")
        _must(payload.get("go_no_go") == "GO", "beta-dress-rehearsal did not emit GO outcome.")
        ...
        _must(payload.get("failed_steps") == [], "beta-dress-rehearsal reported unexpected failed steps.")
        evidence_path = Path(str(payload.get("evidence_path", "")))
        report_path = Path(str(payload.get("report_output_path", "")))
        _must(evidence_path.exists(), "beta-dress-rehearsal evidence JSON was not written.")
        _must(report_path.exists(), "beta-dress-rehearsal markdown report was not generated.")
