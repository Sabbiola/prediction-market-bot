...
def test_smoke_live_data_command_fetches_and_persists_batch(
    ...
    event = next(
        row
        for row in audit_rows
        if row.get("run_id") == deterministic_run_id and row.get("event_type") == "live_market_fetch_end"
    )
    payload = event.get("payload", {})
    assert isinstance(payload, dict)
    assert float(payload.get("fetch_duration_ms", 0.0)) >= 0.0
    assert float(payload.get("total_duration_ms", 0.0)) >= 0.0
