...
from prediction_market_bot.strategy_research.training import service as training_service
...
def test_training_lab_reports_xgboost_skip_reason_explicitly(
    tmp_path: Path,
    ...
    dataset_id = "ds-1"
    _seed_feature_rows(base_dir, dataset_id)
    reason = "xgboost candidate training failed (RuntimeError): simulated failure"
    monkeypatch.setattr(
        training_service,
        "train_xgboost_candidate",
        lambda rows, feature_names: (None, reason),
    )
    service = StrategyTrainingLabService(
        historical_base_dir=base_dir,
        default_dataset_id=dataset_id,
    ...
    summary = service.train_baseline_models(include_xgboost=True)

    xgboost_result = summary.models["xgboost_candidate"]
    assert xgboost_result.status == "skipped"
    assert xgboost_result.reason == reason
    assert reason in summary.warnings
